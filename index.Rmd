...
This should return the vector of colours you input. You can then rebuild the documentation by running
...
This should result in all of your tests passing. If they don't pass, check the error message, make sure your new colour palette is written in valid HEX and make sure you have put it in the right place alphabetically in the list.
...
Notice a couple of things. Firstly, this is a vector, so to add a new entry you have to put a comma after the previous entry (this is easily forgotten). Secondly, you do not have to supply an email address as people will not be contacting you about package maintenance. Thirdly, the role has been changed from `role = c("aut", "cre")` to `role = c("ctb")`, this means you will be recognised as a **c**on**t**ri**b**utor to the package (not an **aut**hor or a **cre**ator). Finally, you don't have to add an ORCID ID if you don't have one, this can be left blank.
